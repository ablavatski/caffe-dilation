...
// Read the file with filenames and labels
  ...
  CHECK_EQ(bin_names_.size(), roi_names_.size());
...
// randomly shuffle data
  ...
  cv::Mat bin_image = SliceChannels(ReadImage(bin_names_[0]), data_param.bin_slice());
...
void BinROILabelDataLayer<Dtype>::load_batch(Batch<Dtype>* batch) {
  ...
  cv::Mat bin_image = SliceChannels(ReadImage(bin_names_[lines_id_]),
...
//                 new_height, new_width, prefetch_data + image_offset);
    std::string image_path = bin_names_[lines_id_];
